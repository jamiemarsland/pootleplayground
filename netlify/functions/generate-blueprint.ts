...
export const handler: Handler = async (event: HandlerEvent, context: HandlerContext) => {
  ...
  try {
    const { prompt, context: userContext }: BlueprintRequest = JSON.parse(event.body || '{}');
    ...
    if (!openaiApiKey) {
      ...
      return {
        ...
        headers,
        body: JSON.stringify({
          error: 'AI service not configured. Please add your OPENAI_API_KEY to project secrets.',
          details: 'The OpenAI API key is missing from environment variables.'
        }),
      };
    }

    const systemPrompt = userContext || `You are an expert WordPress Blueprint generator for the Pootle Playground tool. Your task is to convert user descriptions into step configurations.
...
Remember: Output ONLY the JSON object. No markdown formatting, no explanations, no code blocks.`;
    ...
    if (!openaiResponse.ok) {
      ...
      return {
        statusCode: 500,
        headers,
        body: JSON.stringify({ error: 'Failed to generate blueprint from AI service' }),
      };
    }
